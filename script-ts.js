...
var rec = document.getElementById("rec2");
...
const url = "file/akreditasi_sd.pdf";

pdfjsLib.getDocument(url).promise.then(function (pdfDoc) {
  // Get the first page
  pdfDoc.getPage(1).then(function (page) {
    const scale = 1.5; // Adjust the scale to fit your needs
    const viewport = page.getViewport({ scale: scale });

    // Prepare canvas using PDF page dimensions
    const canvas = document.getElementById("akreditasi_sd");
